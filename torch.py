...
class ChessCNN(nn.Module):
    def __init__(self, env, cnn_channels=32, hidden_size=256, **kwargs):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(18, cnn_channels, 5, stride=3)),
            nn.ReLU(),
        ...
        self.proj = nn.Sequential(
            layer_init(nn.Linear(encode_dim, hidden_size)),
            nn.ReLU(),
        )
    ...
    def encode_observations(self, observations, state=None):
        ...
        castle_byte = obs[:, 769].long()
        castle_planes = torch.zeros(B, 4, 8, 8, device=observations.device)
        for i in range(4):
            castle_planes[:, i] = ((castle_byte & (1 << i)) > 0).float().unsqueeze(1).unsqueeze(1).expand(-1, 8, 8)
        
        ep_sq = obs[:, 770].long()
        ep_plane = torch.zeros(B, 1, 8, 8, device=observations.device)
        valid_ep = (ep_sq < 64)
        if valid_ep.any():
            ep_file = ep_sq[valid_ep] % 8
            ep_rank = ep_sq[valid_ep] // 8
            batch_idx = torch.where(valid_ep)[0]
            ep_plane[batch_idx, 0, ep_rank, ep_file] = 1.0
        
        selected_piece = obs[:, 772:836].view(B, 1, 8, 8)
        
        board_input = torch.cat([board, castle_planes, ep_plane, selected_piece], dim=1)
...
class PointerPolicy(nn.Module):
    """Chess policy with optional 1-ply value-based search.
    
    Two modes:
    1. Pure policy (fast): Direct logits from network
    2. Search mode (training): 1-ply lookahead using value network
    
    Search improves sample efficiency by providing richer targets.
    """
    def __init__(self, env, hidden_size: int = 276, temperature: float = 1.0, cnn_channels: int = 32, **kwargs):
        super().__init__()
        self.hidden_size = hidden_size
        self.is_continuous = False
        self.temperature = float(temperature) if temperature and float(temperature) > 0 else 1.0

        # Observation layout from chess.h - OPTIMIZED (Ocean best practice)
        self.BOARD_SIZE = 768        # 12 planes × 64 squares (pre-computed in C)
        self.MASK_SIZE = 64          # Max legal moves
        self.MAX_MOVES = 64          # Chess maximum
        self.OBS_SIZE = 1156         # Total observation size
        
        # Snake-style CNN: one-hot pieces (12 planes) + castling (4 bits as planes) + EP (1 plane) = 17 channels
        # Very shallow and fast like Snake env
        self.board_cnn = nn.Sequential(
            layer_init(nn.Conv2d(17, cnn_channels, kernel_size=3, padding=1)),  # 8x8→8x8, preserve spatial
            nn.ReLU(),
            nn.Flatten(),  # [B, 32*64] = [B, 2048]
            layer_init(nn.Linear(cnn_channels * 64, hidden_size)),
        )
        
        # Hierarchical action selection:
        # Layer 0: Which piece type? (Pawn=0, Knight=1, Bishop=2, Rook=3, Queen=4, King=5)
        self.piece_type_head = layer_init(nn.Linear(hidden_size, 6), std=0.01)
        
        # Layer 1: Which specific move for that piece?
        self.W_move = layer_init(nn.Linear(5, hidden_size))
        self.W_ctx = layer_init(nn.Linear(hidden_size, hidden_size))
        self.v_attn = layer_init(nn.Linear(hidden_size, 1), std=0.01)
        
        # Value head
        self.value_head = layer_init(nn.Linear(hidden_size, 1), std=1)

    def forward(self, observations, state=None):
        """Complete forward pass - used when no LSTM wrapper."""
        hidden = self.encode_observations(observations, state)
        return self.decode_actions(hidden, state, observations)

    def encode_observations(self, observations, state=None):
        """Encode board to hidden state for LSTM.
        
        Cache observations so decode_actions can extract move features.
        """
        if state is not None:
            state['observations'] = observations
        # Cache for decode_actions (LSTM doesn't pass observations)
        self._obs_cache = observations
        
        B = observations.shape[0]
        
        # C provides 12 one-hot planes: [B, 768] → [B, 12, 8, 8]
        board_planes = observations[:, :768].view(B, 12, 8, 8).float()
        
        # Castling rights: 4 bits → 4 binary planes (K, Q, k, q)
        castle_bits = observations[:, 769].unsqueeze(1)  # [B, 1]
        castle_K = ((castle_bits & 1) > 0).float().view(B, 1, 1, 1).expand(B, 1, 8, 8)
        castle_Q = ((castle_bits & 2) > 0).float().view(B, 1, 1, 1).expand(B, 1, 8, 8)
        castle_k = ((castle_bits & 4) > 0).float().view(B, 1, 1, 1).expand(B, 1, 8, 8)
        castle_q = ((castle_bits & 8) > 0).float().view(B, 1, 1, 1).expand(B, 1, 8, 8)
        
        # EP square: binary plane with 1 at EP square if valid
        ep_sq = observations[:, 770].long()
        ep_plane = torch.zeros(B, 1, 8, 8, device=board_planes.device, dtype=board_planes.dtype)
        valid_ep = (ep_sq < 64)
        if valid_ep.any():
            batch_idx = valid_ep.nonzero(as_tuple=False).squeeze(-1)
            ep_ranks = ep_sq[valid_ep] // 8
            ep_files = ep_sq[valid_ep] % 8
            ep_plane[batch_idx, 0, ep_ranks, ep_files] = 1.0
        
        # Concatenate: [B, 17, 8, 8] - 12 pieces + 4 castling + 1 EP
        board_17 = torch.cat([board_planes, castle_K, castle_Q, castle_k, castle_q, ep_plane], dim=1)
        
        # CNN encoding
        return self.board_cnn(board_17)

    def decode_actions(self, hidden, state=None, observations=None):
        """Decode actions from hidden state. Get observations from cache."""
        # Get observations from cache (set in encode_observations)
        if observations is None:
            observations = getattr(self, '_obs_cache', None)
        if observations is None:
            # Fallback: return zero logits (should never happen in practice)
            B = hidden.shape[0]
            logits = hidden.new_zeros((B, self.MAX_MOVES))
        ...
        legal = observations[:, 772:836] > 0
        piece_types = observations[:, 1156:1220].long()  # [B, 64] piece type per move (0-5)
        piece_counts = observations[:, 1220:1226].float()  # [B, 6] moves per piece type
        
        feat_dtype = self.W_move.weight.dtype
        fr = observations[:, 836:900].to(dtype=feat_dtype) / 63.0
        to = observations[:, 900:964].to(dtype=feat_dtype) / 63.0
        moved = observations[:, 964:1028].to(dtype=feat_dtype) / 15.0
        captured = observations[:, 1028:1092].to(dtype=feat_dtype) / 15.0
        promo = observations[:, 1092:1156].to(dtype=feat_dtype) / 15.0
        
        per_move = torch.stack([fr, to, moved, captured, promo], dim=-1).contiguous()
        
        # Layer 0: Piece type logits (for auxiliary loss/reward)
        piece_type_logits = self.piece_type_head(hidden)  # [B, 6]
        # Mask piece types with no legal moves
        piece_mask = (piece_counts > 0)
        piece_type_logits = piece_type_logits.masked_fill(~piece_mask, float('-inf'))
        
        # Layer 1: Specific move selection via pointer attention
        move_proj = self.W_move(per_move)
        ctx_proj = self.W_ctx(hidden).unsqueeze(1)
        move_logits = self.v_attn(torch.tanh(move_proj + ctx_proj)).squeeze(-1)
        
        # Combine hierarchically: bias move logits by piece type preference
        piece_bias = piece_type_logits.gather(1, piece_types.clamp(0, 5))  # [B, 64]
        logits = move_logits + 0.5 * piece_bias  # Blend piece preference into move selection
        
        if self.temperature != 1.0:
            logits = logits / self.temperature
        
        logits = logits.masked_fill_(~legal, float('-inf'))
        
        # Handle no legal moves
        has_legal = legal.any(dim=-1)
        logits[:, 0] = torch.where(has_legal, logits[:, 0], logits.new_zeros(B))
        
        value = self.value_head(hidden).squeeze(-1)
        
        # Return both move logits (for action) and piece_type_logits (for auxiliary reward)
        # Store piece_type_logits for potential auxiliary loss
        return logits, value
